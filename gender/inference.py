...
def inference(data_dir, model_dir, output_dir, args):
    ...
    num_classes = MaskBaseDataset.num_classes  # 2
    model = load_model(model_dir, num_classes, device).to(device)
    model.eval()

    img_root = os.path.join(data_dir, 'crop_images')###crop
...
#             pred = torch.round(torch.sigmoid(pred).squeeze()) ## BCELoss
            preds.extend(pred.cpu().numpy())

    info['ans'] = preds
#     info['ans'] = info['ans'].astype(int) ## BCELoss
