...
class MaskBaseDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.task == 'mask': 
            label = self.get_mask_label(index)
        elif self.task == 'gender':
            label = self.get_gender_label(index)
        elif self.task == 'age':
            label = self.get_age_label(index)
...
class TestDataset(Dataset):
    def __init__(self, img_paths, resize, mean=(0.548, 0.504, 0.479), std=(0.237, 0.247, 0.246), task='mask'):
